...
#+begin_src emacs-lisp
  ...
  ;;   for GNU Emacs.
  ;;
  ;;   I think Emacs' corutines must be in C core
  ;;   or written in Emacs Lisp with C module.
  ;;   (Core/Module can be written with `libuv' C library
  ;;   *Used in NodeJS*)
  ;;   
  ;;   For now we can just use aio.el library.
  ;;   
  ;;   Also aio.el must be a example of async i/o
  ;;   library for GNU Emacs.
  ;;
...
Pomodoro техника для org-mode

#+begin_src emacs-lisp
  (setq-default org-pomodoro-length 25
                org-pomodoro-short-break-length 5
                org-pomodoro-long-break-length 20)

  ;; FIXME: Pomodoro bindings doesn't work.
  ...
  (elpaca 'org-pomodoro
    ...
    (bind-key "o p" 'org-pomodoro mode-specific-map))
...
*** Meson (Without Tree-Sitter)

#+begin_src emacs-lisp
  (elpaca 'meson-mode
    ;; NOTE: All necessary autoloads is loaded automatically from source code.
    )
#+end_src
